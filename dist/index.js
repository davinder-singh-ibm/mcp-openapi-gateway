...
async function main() {
    try {
        ...
        const loadedSpecs = await loadAllSwaggerSpecs(swaggerConfigs, config.swaggerTimeoutMs, config.allowInsecureTls);
        ...
        const generatedTools = generateAllTools(loadedSpecs, config.toolNamePrefix, config.serviceNameMode, config.defaultServiceName, config.maxTools);
        ...
        if (config.swaggerRefreshSeconds) {
            ...
            setInterval(async () => {
                try {
                    ...
                    const refreshedSpecs = await loadAllSwaggerSpecs(swaggerConfigs, config.swaggerTimeoutMs, config.allowInsecureTls);
                    validateAllSpecs(refreshedSpecs);
                    const refreshedTools = generateAllTools(refreshedSpecs, config.toolNamePrefix, config.serviceNameMode, config.defaultServiceName, config.maxTools);
